<template>
    <div class="border-2 border-solid border-green-600 max-w-sm p-5 min-w-80 min-h-screen w-96 transition-all	">
        <div class="flex items-center justify-between m-3 mt-0 mb-5">
            <h1 class="text-4xl">{{ stage.titleStage }}</h1>
            <div class="flex gap-3 cursor-pointer">
                <svg @click="editStage()" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                </svg>
                <svg @click="deleteStage()" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round"
...
const showModalEditStage = () => {
    rootStore.modalVisibleEditStage = true
}
const editStage = () => {
    showModalEditStage()
